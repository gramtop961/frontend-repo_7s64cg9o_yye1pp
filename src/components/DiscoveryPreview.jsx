...
export default function DiscoveryPreview({ onOpenFilters, onSayHi, onViewProfile, onJoinTrip, onTripDetails }) {
  const nearby = [
    ...
    <section className="bg-neutral-950 text-white py-14 sm:py-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={onOpenFilters} className="inline-flex items-center gap-2 text-sm rounded-lg border border-white/15 bg-white/5 px-3 py-2 hover:bg-white/10 transition">
              ...
              {nearby.map((p) => (
                <div key={p.name} className="rounded-xl border border-white/10 bg-black/30 p-4">
                  ...
                  <div className="mt-3 flex gap-2">
                    <button onClick={() => onSayHi?.(p.name)} className="text-xs rounded-md bg-white text-black px-3 py-1.5 font-medium hover:bg-orange-100">Say hi</button>
                    <button onClick={() => onViewProfile?.(p.name)} className="text-xs rounded-md border border-white/15 bg-white/5 px-3 py-1.5 hover:bg-white/10">View</button>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-3">
              {trips.map((t) => (
                <div key={t.route} className="rounded-xl border border-white/10 bg-black/30 p-4">
                  ...
                  <div className="mt-3 flex gap-2">
                    <button onClick={() => onJoinTrip?.(t.route)} className="text-xs rounded-md bg-white text-black px-3 py-1.5 font-medium hover:bg-orange-100">Join</button>
                    <button onClick={() => onTripDetails?.(t.route, t.dates, t.match)} className="text-xs rounded-md border border-white/15 bg-white/5 px-3 py-1.5 hover:bg-white/10">Details</button>
                  </div>
                </div>
              ))}
            </div>
